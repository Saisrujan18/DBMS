...
export default function Product() {
					...
					<div className="flex flex-row items-center mr-2 text-sm font-medium text-gray-900">
						...
						{index !== path.length - 1 && (
							<svg
								...
								aria-hidden="true"
								className="ml-2 w-4 h-5 text-gray-300"
							>
	...
	const Star = (props) => {
		return (
			<svg
				className={`text-${props.color} h-5 w-5 flex-shrink-0`}
				xmlns="http://www.w3.org/2000/svg"
				viewBox="0 0 20 20"
				fill="currentColor"
				aria-hidden="true"
	...
	const GetRating = (props) => {
		...
		let stars_svg = [];
		for (let i = 0; i < stars; i++) stars_svg.push(<Star color="gray-900" />);
		...
		return (
			<div className="mt-6">
				...
				<div className="flex items-center">
					<div className="flex items-center">{stars_svg}</div>
				</div>
			</div>
		);
	};
	const SingleColor = (props) => {
		return (
			<label className="-m-0.5 relative p-0.5 rounded-full flex items-center justify-center cursor-pointer focus:outline-none ring-gray-400">
				...
				<span
					aria-hidden="true"
					className={`h-8 w-8 bg-${props.color[0]}${
						props.color.length > 1 ? -props.color[1] : ""
			...
			<div className="py-10 lg:pt-6 lg:pb-16 lg:col-start-1 lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
				...
				<div className="mt-10">
					...
					{product.description.length > 1 &&
						product.description[1].length > 0 && (
							<Highlights content={product.description[1]} />
						)}
				</div>
